...
qdot_v = zeros(length(t), 7);
xdot_v = zeros(length(t), 6);
errors = zeros(length(t), 6);
ee_vels = zeros(length(t), 6);
t_vels = zeros(length(t), 6);
index = 1;
...
for i = t
    ...
    [b_e, x_dot] = cc.getCartesianReference(bTg);
    errors(index, :) = b_e;
    ...
    t_vels(index, :) = km.J * q_dot;
    ee_vels(index, :) = km.getJacobianOfLinkWrtBase(gm.jointNumber) * q_dot;
...
disp([ee_vels(1,:)', ee_vels(index - 1, :)'])

disp("Inital and final tool velocities")
disp([t_vels(1,:)', t_vels(index - 1, :)'])
